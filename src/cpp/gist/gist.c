 ...
 *
 * Modify by leo, leoyonn@gmail.com, 2012-11-5
 */
/*--------------------------------------------------------------------------*/
...
static void color_down_N(double *res, color_image_t *src, int N, int c)
{
    int i, j, k, l;
...
static double *color_gist_gabor(color_image_t *src, const int w, image_list_t *G)
{
    fftw_lock();
    ...
    double *res = (double *) malloc(3*w*w*G->size*sizeof(double));
...
double *color_gist(color_image_t *src, int w, int a, int b, int c) {
    int orientationsPerScale[3];
...
double *color_gist_scaletab(color_image_t *src, int w, int n_scale, const int *n_orientation)
{
    int i;
    ...
    double *g = color_gist_gabor(img, numberBlocks, G);
